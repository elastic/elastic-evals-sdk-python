...
from elastic_evals.reporting.compare import compare_runs, pair_scores, print_comparison_report
...
@click.option("--es-url", envvar="EVALUATIONS_ES_URL", required=True, help="Elasticsearch URL.")
@click.option("--significance", default=0.05, show_default=True, help="P-value threshold.")
def compare_cmd(run_id_1: str, run_id_2: str, es_url: str, significance: float) -> None:
    ...
    async def _compare() -> None:
        ...
        datasets_a = {score.example.dataset.id: score.example.dataset.name for score in scores_a}
        datasets_b = {score.example.dataset.id: score.example.dataset.name for score in scores_b}
        overlap = set(datasets_a).intersection(datasets_b)
        if not overlap:
            raise click.ClickException("No overlapping datasets found between the two runs.")

        filtered_a = [score for score in scores_a if score.example.dataset.id in overlap]
        filtered_b = [score for score in scores_b if score.example.dataset.id in overlap]
