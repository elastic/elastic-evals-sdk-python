...
# Load .env file before importing config-dependent modules
_load_dotenv()
...
def extract_ground_truth(expected: dict[str, Any]) -> dict[str, dict[str, float]]:
    ...
    if "relevant_docs" in expected:
        ...
        return {
            index: {doc_id: 1.0 for doc_id in expected["relevant_docs"]}
        }
...
def create_evaluators(
    ...
    if trace_es_client:
        evaluators.append(create_latency_evaluator(
            trace_es_client=trace_es_client, log=log
        ))
        evaluators.append(create_input_tokens_evaluator(
            trace_es_client=trace_es_client, log=log
        ))
        evaluators.append(create_output_tokens_evaluator(
            trace_es_client=trace_es_client, log=log
        ))
        evaluators.append(create_tool_calls_evaluator(
            trace_es_client=trace_es_client, log=log
        ))
...
def build_dataset_scores_with_stats(
    dataset: EvaluationDataset,
    experiment,
    evaluator_names: list[str],
    repetitions: int,
) -> DatasetScoreWithStats:
    """Build dataset scores with statistics for reporting."""
    total_examples = len(dataset.examples) * repetitions
    ...
    return DatasetScoreWithStats(
        ...
        name=experiment.dataset_name or dataset.name,
...
async def main() -> None:
    ...
    # 8. Export to Elasticsearch (if configured)
    ...
    dataset_scores = [
        build_dataset_scores_with_stats(
            dataset=dataset,
            experiment=result,
    ...
    from elastic_evals.reporting.types import ReportDisplayOptions, EvaluatorDisplayOptions
